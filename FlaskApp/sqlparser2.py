...
def parser(database):
    ...
    file1=open('sql_metadata/'+database+'/'+database+'.sql')
    ...
    import json
    import pickle
    with open('sql_metadata/'+database+'/table_attributes.json', 'w') as f:
        json.dump(table_attributes, f)
    with open('sql_metadata/'+database+'/tables_pk.json', 'w') as f:
        json.dump(tables_pk, f)
    # elsewhere...
    pickle.dump( tables_relation, open( "sql_metadata/"+database+"/tables_relation.p", "wb" ))
    pickle.dump(tables,open("sql_metadata/"+database+"/tables.p","wb"))
